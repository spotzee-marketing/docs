...
Call `GET /generic/email/email-tester`. The API returns `{ test_email, uniqueCode }`. The address looks like `<uniqueCode>@spotzee.marketing` and is valid for one captured message. The Spotzee API captures the inbound message at that address for analysis.
</Step>
