...
Every list endpoint in the Main API uses **cursor pagination**. Cursors are stable across inserts and deletes, so you never miss or duplicate a record while paging.
...
For v1, use simple `?key=value` filters where supported. The fielded query language (AIP-160 syntax) is on the roadmap.
...
<Columns cols={2}>
  <Card title="Rate limits" icon="gauge" href="/main-api/rate-limits">Stay under your per-second budget while paging.</Card>
