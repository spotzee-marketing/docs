...
<Steps>
  ...
  <Step title="Compare with constant-time equality">
    Use a constant-time comparison (e.g. `crypto.timingSafeEqual` in Node, `hmac.compare_digest` in Python). Reject on mismatch.
  </Step>
</Steps>
...
If signature verification fails on Spotzee's side (e.g. you flipped the secret without rotating), we surface `webhook_signature_invalid`:
...
After the final attempt, the delivery is marked failed and surfaced in **Settings → Webhooks → Deliveries**. Replies with `2xx` (any) confirm receipt; `4xx` is a permanent failure and **stops** retrying.

## Idempotency on the receiving side

Webhooks are at-least-once. Stamp each event with its `event_id` and discard duplicates on your side, or use the `Idempotency-Key`-style pattern in your own database to make the handler safe to call twice.

## Next

<Columns cols={2}>
  <Card title="Errors" icon="circle-exclamation" href="/main-api/errors">Status codes and the `code` catalogue.</Card>
  <Card title="Authentication" icon="key" href="/main-api/authentication">Choose the right key type for outgoing API calls.</Card>
</Columns>
