---
title: "Rate limits"
description: "Per-key rate limits, response headers, and retry strategy for the Spotzee Main API."
...
Read = `GET`, `HEAD`, `OPTIONS`. Write = `POST`, `PATCH`, `PUT`, `DELETE`.

### MCP and agent workloads

If you're driving the API from an MCP server or agent runtime, send the `x-spotzee-client-type: mcp` header. We **double** your base limits when this header is present so parallel tool calls don't run out of budget mid-task.
